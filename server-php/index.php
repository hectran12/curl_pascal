...
// example url: http://localhost:6778/?server=on&url=https://raw.githubusercontent.com/hexzzz2008/HexRequestAction-Pascal/main/server_info.json&method=GET&prot=CURLOPT_RETURNTRANSFER|true,CURLOPT_USERAGENT|concu&return=curl_close|self,curl_exec|self&headers=encodinglmao,buhbuh
// $ch = curl_init();
// curl_setopt($ch, CURLOPT_URL, "https://raw.githubusercontent.com/hexzzz2008/HexRequestAction-Pascal/main/server_info.json");
// curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
// curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
// curl_setopt($ch, CURLOPT_USERAGENT, "concu");
// curl_setopt($ch, CURLOPT_HTTPHEADER, array("encodinglmao","buhbuh",));
// $arrCode = array();
// $arrCode["curl_close"] = curl_close($ch);
// $arrCode["curl_exec"] = curl_exec($ch);
...
if (isset($_GET["server"]) == "on") {
    ...
    if (isset($_GET["method"]) && isset($_GET["prot"]) && isset($_GET["return"]) && isset($_GET["url"])) {
        ...
        $method = decode($method[strlen($method)-1] == ',' ? substr($method, 0, -1) : $method);
        $prot = decode($prot[strlen($prot)-1] == ',' ? substr($prot, 0, -1) : $prot);
        ...
        echo '<b>Protocol:</b> ' . $prot . '<br>';
        echo '<b>Return:</b> ' . $return . '<br>';
        echo '<b>URL:</b> ' . $url . '<br>';
        echo '</pre>';
        ...
        try {
            if(strpos($prot, "|")) {
                ...
                if ($checkProt) {
                    ...
                    if ($header) {
                        $listheader = checkHeader($header);
                        if ($listheader) {
                            createHeader($code);
                            foreach ($listheader as $prot) {
                                addHeader($code, $prot);
                            }
                            finalHeader($code);
                            
                        } else {
                            outError("Header inserted has failed, invalid");
                        }
                    }
